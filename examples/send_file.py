...
def get_meshtastic_interface(device_port=None):
    ...
    try:
        ...
        print("Ensure the device is connected, powered on, and permissions are correct (e.g., user in 'dialout' group on Linux).")
        sys.exit(1)
...
def dummy_save_file(filename, data):
    ...
    print(f"[SENDER SCRIPT WARNING] Dummy save callback invoked for file '{filename}' - this indicates an unexpected incoming transfer.")
    pass
...
def on_receive_akita(packet, interface):
    ...
    """
    global sender, receiver # Allow access to global instances
...
def main():
    ...
    parser.add_argument("-p", "--port", help="Serial port or device path for Meshtastic device (e.g., /dev/ttyUSB0, COM3). Default: Auto-detect.")
    parser.add_argument("--piece-size", type=int, default=DEFAULT_PIECE_SIZE, help=f"Size of file pieces in bytes (default: {DEFAULT_PIECE_SIZE}).")
    parser.add_argument("--no-merkle", action="store_true", help="Disable Merkle root, send individual hashes instead.")
    parser.add_argument("--delay", type=float, default=0.2, help="Initial delay between sending pieces (seconds, default: 0.2).")
    parser.add_argument("--timeout", type=float, default=300.0, help="Overall transfer timeout in seconds (default: 300). Sender gives up if no completion ACK after this time.")
