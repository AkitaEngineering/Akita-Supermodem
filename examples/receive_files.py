...
def on_receive_akita(packet, interface):
    ...
    if payload and portnum == AKITA_CONTENT_TYPE:
        try:
            ...
            print(
                f"\n[Akita RX] Received Akita message from {sender_node_id} "
                f"({sender_node_num or 'N/A'}) {'(Broadcast)' if is_broadcast else ''}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run the Akita Supermodem receiver to listen for files over Meshtastic."
    )
    ...
    parser.add_argument(
        ...
        help=f"Timeout value (currently used for inactivity check, default: {DEFAULT_TIMEOUT}s).",
    )
    parser.add_argument(
        "--retries",
        type=int,
        default=DEFAULT_MAX_RETRIES,
        help=f"Max times to request a missing piece (default: {DEFAULT_MAX_RETRIES}).",
    )
    parser.add_argument(
        "--interval",
        type=float,
        default=15.0,
        help="Seconds between periodic checks for sending missing piece requests (default: 15.0).",
    )
